...
export function useSupervisorMeetingRecordsState(
      ...
      try {
        if (formMode === "add") {
          ...
          openSuccessModal(
            ...
            "Meeting record was added successfully.",
          );
        } else {
          await updateRecord(editingRecord!.id, payload);
          openSuccessModal(
            "Meeting record updated",
            "Meeting record was updated successfully.",
          );
        }
        closeForm();
      } catch (caught) {
  ...
  const confirmDelete = useCallback(async () => {
    ...
    openLoadingModal(
      ...
      "Removing meeting record from this project.",
    );
    try {
      ...
      openSuccessModal(
        ...
        "Meeting record was removed successfully.",
      );
    } catch (caught) {
      const apiError = toApiError(
        caught,
        "Unable to delete meeting record right now.",
      );
      openErrorModal(
        "Unable to delete meeting record",
        apiError,
        () => void confirmDelete(),
      );
    }
  ...
  const approve = useCallback(
    async (record: MeetingRecord) => {
      openLoadingModal(
        "Approving meeting record",
        "Approving the selected meeting record.",
      );
      try {
        await approveRecord(record.id);
        openSuccessModal(
          "Meeting record approved",
          "Meeting record was approved successfully.",
        );
      } catch (caught) {
        const apiError = toApiError(
          caught,
          "Unable to approve meeting record right now.",
        );
        openErrorModal(
          "Unable to approve meeting record",
          apiError,
          () => void approve(record),
        );
      }

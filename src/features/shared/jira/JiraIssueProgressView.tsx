...
export function JiraIssueProgressView({ projectId, fetcher, refresher }: Props) {
  ...
  useEffect(() => {
    // Start with top-level work expanded. Deep child groups remain user-controlled.
    setExpanded(new Set(tree.roots.filter((node) => node.children.length).map((node) => node.issue.issueKey)));
  }, [data?.items]);

  const normalizedQuery = query.trim().toLowerCase();
  const rows = useMemo(() => {
    const result: VisibleRow[] = [];
    const matchesSelf = (issue: JiraIssue) => !normalizedQuery || [issue.issueKey, issue.summary, issue.issueType, issue.status, issue.assigneeDisplayName ?? "", issue.priority ?? ""].some((v) => v.toLowerCase().includes(normalizedQuery));
    ...
    const walk = (node: TreeNode, depth: number) => {
      ...
      result.push({ node, depth, matched: normalizedQuery.length > 0 && selfMatch });
      const shouldOpen = normalizedQuery.length > 0 || expanded.has(node.issue.issueKey);
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-4">
        {[["Total", data.summary.total], ["To do", data.summary.toDo], ["In progress", data.summary.inProgress], ["Done", data.summary.done]].map(([label, value]) => <div key={String(label)} className="rounded-2xl border border-slate-200 bg-white p-4 text-center"><div className="text-sm text-slate-500">{label}</div><div className="mt-1 text-2xl font-semibold text-slate-900">{value}</div></div>)}
      </div>
            ...
            <table className="min-w-full text-left text-sm">
              ...
              <tbody>
                {rows.map(({ node, depth, matched }) => {
                  ...
                  return <tr key={issue.issueKey} className={`group border-b border-slate-100 last:border-0 transition-colors hover:bg-slate-50/70 ${matched ? "bg-amber-50/40" : ""}`}>
                    ...
                    <td className="px-4 py-3 align-top text-slate-600">{issue.assigneeDisplayName ?? "Unassigned"}</td>
                  </tr>;
                })}

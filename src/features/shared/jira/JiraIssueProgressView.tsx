...
import { JiraHierarchySkeleton } from "@/features/supervisor/components/ProjectDetail/jira/JiraHierarchySkeleton";
...
function compareNodes(a: TreeNode, b: TreeNode) {
  return (
    ...
    keyNumber(a.issue.issueKey) - keyNumber(b.issue.issueKey) ||
    a.issue.issueKey.localeCompare(b.issue.issueKey)
  );
}
...
export function JiraIssueProgressView({
  ...
  const rows = useMemo(() => {
    ...
    const matchesType = (issue: JiraIssue) =>
      typeFilter === "all" ||
      issue.issueType.toLowerCase() === typeFilter ||
      (typeFilter === "subtask" &&
        (issue.isSubtask || issue.issueType.toLowerCase().includes("sub")));
    const matchesText = (issue: JiraIssue) =>
      !normalizedQuery ||
      [
        issue.issueKey,
        issue.summary,
    ...
    const matchesSelf = (issue: JiraIssue) =>
      matchesType(issue) && matchesText(issue);
    const containsMatch = (node: TreeNode): boolean =>
      matchesSelf(node.issue) || node.children.some(containsMatch);
  ...
  if (loading && !data) return <JiraHierarchySkeleton />;
  if (error && !data)
    return (
    ...
    <section className="space-y-3">
      ...
      {neverSynced ? (
        <EmptyState
          ...
          description="Refresh Jira to create the local ResearchTrack issue snapshot."
        />
      ) : data.items.length === 0 ? (
        <EmptyState
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-white px-4 py-3">
            <div className="flex items-center gap-2 text-xs text-slate-500">
              <span className="rounded-full bg-slate-100 px-2.5 py-1 font-medium text-slate-700">
                {rootCount} root items
              </span>
              <span>{childCount} nested items</span>
              {tree.orphans.size ? (
                <span>· {tree.orphans.size} parent(s) outside snapshot</span>
              ) : null}
            </div>
            <div className="flex flex-1 flex-wrap items-center justify-end gap-2">
              <select
                ...
                className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 outline-none transition focus:border-slate-300 focus:bg-white focus:ring-2 focus:ring-slate-100"
                aria-label="Filter Jira issues by type"
              >
              ...
              <button
                type="button"
                onClick={() =>
                  setExpanded(expanded.size ? new Set() : new Set(expandable))
                }
                className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 px-3 py-2 text-xs font-medium text-slate-600 transition hover:bg-slate-50"
              >
                {expanded.size ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              <thead className="border-b border-slate-200 bg-slate-50/80 text-xs font-semibold uppercase tracking-wide text-slate-500">
                <tr>
                  <th className="w-32 px-4 py-3">Key</th>
                  <th className="min-w-[420px] px-4 py-3">Issue hierarchy</th>
              ...
              <tbody>
                {rows.map(({ node, depth, matched }) => {
                  ...
                  const isOpen =
                    normalizedQuery.length > 0 || expanded.has(issue.issueKey);
                  const toggle = () => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 align-top">
                        <div
                          className="relative flex min-h-10 items-start"
                          style={{
                            paddingLeft: `${Math.min(depth, 6) * 28}px`,
                          }}
                        >
                          {depth > 0 ? (
                            <span
                              className="absolute top-0 h-5 w-4 rounded-bl-lg border-b border-l border-slate-200"
                              style={{
                                left: `${Math.min(depth, 6) * 28 - 18}px`,
                              }}
                              aria-hidden="true"
                            />
                          ) : null}
                          <button
                            ...
                            className={`mr-2 mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-md transition ${hasChildren ? "text-slate-500 hover:bg-white/80 hover:text-slate-900" : "cursor-default text-slate-300"}`}
                            aria-label={
                              hasChildren
                                ? `${isOpen ? "Collapse" : "Expand"} ${issue.issueKey}`
                                : undefined
                            }
                          >
                            {hasChildren ? (
                          ...
                          <div className="min-w-0">
                            <button
                              type="button"
                              onClick={() => setSelectedIssue(issue)}
                              className="text-left font-medium leading-5 text-slate-900 underline-offset-2 hover:text-blue-700 hover:underline"
                            >
                              {issue.summary}
                            </button>
                            <div className="mt-1 flex flex-wrap items-center gap-1.5">
                              <span
                                className={`rounded-full px-2 py-0.5 text-[11px] font-medium ring-1 ring-inset ${typeTone(issue.issueType)}`}
                              >
                                {issue.issueType}
                              </span>
                              ...
                              {tree.orphans.has(issue.issueKey) ? (
                                <span className="text-xs text-amber-600">
                                  · parent {issue.parentIssueKey} not in
                                  snapshot
                                </span>
                              ) : null}
                            </div>
                      ...
                      <td className="px-4 py-3 align-top text-slate-600">
                        <JiraContributorIdentity
                          accountId={
                            issue.assigneeAccountId ?? issue.assigneeDisplayName
                          }
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center">
                      ...
                      <div className="mt-1 text-sm text-slate-500">
                        Try another key, title, status, priority, or assignee.
                      </div>
                    </td>
                  </tr>
                ) : null}

...
export function JiraIssueProgressView({ projectId, fetcher }: Props) {
  ...
  return (
    <section className="space-y-3">
      ...
      {neverSynced ? <EmptyState title="Jira issues have not been synchronized yet" description="Refresh Jira to create the local ResearchTrack issue snapshot." /> : data.items.length === 0 ? <EmptyState title="No synchronized Jira issues" description="The linked Jira project currently has no issues available in the ResearchTrack snapshot." /> : (

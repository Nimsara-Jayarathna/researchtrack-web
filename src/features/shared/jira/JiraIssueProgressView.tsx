...
export function JiraIssueProgressView({
  ...
  const rows = useMemo(() => {
    ...
    const walk = (node: TreeNode, depth: number) => {
      ...
      if (normalizedQuery && !containsMatch(node)) return;
      result.push({
        node,
        depth,
        matched: normalizedQuery.length > 0 && selfMatch,
      });
      const shouldOpen =
        normalizedQuery.length > 0 || expanded.has(node.issue.issueKey);
      if (shouldOpen) node.children.forEach((child) => walk(child, depth + 1));
    };
    tree.roots.forEach((root) => walk(root, 0));
    return result;
  }, [tree.roots, normalizedQuery, expanded]);
  ...
  if (loading && !data)
    return (
      <BlockingState
        isActive
        message="Loading synchronized Jira issues…"
        className="min-h-40"
      />
    );
  if (error && !data)
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Hierarchical ResearchTrack Jira snapshot
          </p>
          <div className="mt-2">
              ...
              <tbody>
                {rows.map(({ node, depth, matched }) => {
                  ...
                  return (
                    <tr
                      key={issue.issueKey}
                      className={`group border-b border-slate-100 last:border-0 transition-colors hover:bg-slate-50/80 ${matched ? "bg-amber-50/60" : depth === 0 && issue.issueType.toLowerCase() === "epic" ? "bg-violet-50/30" : depth === 0 ? "bg-slate-50/40" : "bg-white"}`}
                    >
                      ...
                      <td className="px-4 py-3 align-top">
                        ...
                        >
                          ...
                          <button
                            ...
                            className={`mr-2 mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-md transition ${hasChildren ? "text-slate-500 hover:bg-slate-100 hover:text-slate-900" : "cursor-default text-slate-300"}`}
                            aria-label={
                              hasChildren
                      ...
                      <td className="px-4 py-3 align-top text-slate-600">
                        <JiraContributorIdentity
                          accountId={issue.assigneeDisplayName}

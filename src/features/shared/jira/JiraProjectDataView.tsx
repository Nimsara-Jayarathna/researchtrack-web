...
function cachedFetch<T>(
  ...
  const request = fetcher(projectId)
    .then((data) => {
      cache.set(projectId, { data, cachedAt: Date.now() });
      return data;
    })
...
function invalidate(projectId: string) {
  issueCache.delete(projectId);
  sprintCache.delete(projectId);
  workloadCache.delete(projectId);
}
...
export function JiraProjectDataView({
  ...
  const cachedSprintFetcher = useCallback(
    (id: string) => cachedFetch("sprints", id, sprintCache, sprintFetcher),
    [sprintFetcher],
  );
  const cachedWorkloadFetcher = useCallback(
    ...
    <section className="space-y-3">
      <div className="flex flex-wrap items-center justify-center gap-3 sm:justify-between">
        ...
        {refresher ? (
          ...
          <div className="hidden sm:block sm:w-[132px]" />
        )}
      </div>
      {refreshError ? (

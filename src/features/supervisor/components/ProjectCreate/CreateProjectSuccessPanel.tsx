  ...
  return (
    <section className="rounded-3xl border border-emerald-200 bg-emerald-50 p-6 shadow-sm">
      ...
      <p className="mt-2 text-sm leading-7 text-emerald-800">
        {createdProject.title} was created with {createdProject.students.length}{" "}
        assigned student
        {createdProject.students.length === 1 ? "" : "s"} and the first
        milestone scheduled for{" "}
        {dateFormatter.format(
          new Date(primaryMilestone?.dueDate ?? createdProject.milestoneDate),
        )}
        .
      </p>

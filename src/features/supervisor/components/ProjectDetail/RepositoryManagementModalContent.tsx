...
export function RepositoryManagementModalContent({
  ...
  const sourceHasSyncInProgress = rows.reduce<Record<string, boolean>>(
    (acc, row) => {
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div>
            ...
            <div className="h-2 w-full overflow-hidden rounded-full bg-slate-100 ring-1 ring-inset ring-slate-200">
              <div
                ...
                style={{
                  width: `${Math.min(100, (linkedCount / maxLinkedRepositories) * 100)}%`,
                }}
              />
            </div>
          </div>
        ...
        {bothLimitsReached ? (
          <p className="mt-5 inline-flex items-center rounded-full bg-amber-50 px-3.5 py-1.5 text-[11px] font-semibold text-amber-800 ring-1 ring-inset ring-amber-600/20">
            Linked and enabled limits reached. Unlink one repository and disable
            one enabled repository to continue.
          </p>
        ) : null}
        {!bothLimitsReached && enabledLimitReached ? (
          <p className="mt-5 inline-flex items-center rounded-full bg-amber-50 px-3.5 py-1.5 text-[11px] font-semibold text-amber-800 ring-1 ring-inset ring-amber-600/20">
            Enabled limit reached. Disable one enabled repository before
            enabling another.
          </p>
        ) : null}
        ...
        <>
          <div className="space-y-3 md:hidden">
            {rows.map((row) => {
              ...
              const blockedByEnabledLimit =
                !row.enabled && remainingEnabledSlots < 1;
              const blockedByLinkedLimit =
                !row.enabled && !row.linkId && remainingLinkSlots < 1;
              const enableBlocked =
                blockedByEnabledLimit || blockedByLinkedLimit;
              const sourceSyncing = !!(
                row.sourceId && sourceHasSyncInProgress[row.sourceId]
              );
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2">
                    <div className="rounded-xl border border-slate-200 bg-white px-3 py-2.5">
                      ...
                      <button
                        ...
                        disabled={
                          ...
                          enableBlocked ||
                          (!row.enabled &&
                            (!row.sourceId || !row.githubRepositoryId))
                        }
                        title={
                    ...
                    <div className="rounded-xl border border-slate-200 bg-white px-3 py-2.5">
                      ...
                      <button
                        ...
                        title={
                          row.primary ? "Current primary" : "Set as primary"
                        }
                      >
                        <span
                          className={`inline-block h-5 w-5 rounded-full bg-white shadow-sm transition-transform ${
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    ...
                    {row.linkId ? (
                      <button
                        ...
                        disabled={
                          isMutating || isSavingDisplayName || isSyncing
                        }
                        title={
                          isSyncing
                      ...
                      <button
                        ...
                        disabled={
                          isMutating || isSavingDisplayName || sourceSyncing
                        }
                        title={
                          sourceSyncing

...
function StatCard({ label, value, accent = "neutral" }: StatCardProps) {
  ...
  const statusLabel =
    accent === "red" ? "Critical" : accent === "amber" ? "Watch" : null;
  ...
  return (
    ...
    >
      {railTone ? (
        <div className={`absolute inset-y-0 left-0 w-1 ${railTone}`} />
      ) : null}
...
export function JiraStatCards({ health }: JiraStatCardsProps) {
  ...
  const priorityAccent: StatCardProps["accent"] =
    health.highPriorityOpen > 3
      ? "red"
      : health.highPriorityOpen > 0
        ? "amber"
        : "neutral";
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <StatCard
        label="Overdue"
        value={String(health.overdueIssues)}
